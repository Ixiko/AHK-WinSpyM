...
;----------------------------------------------------------------------------------------------------------------------------------------------;{
    ...
    g_GuiAlwaysOnTop   := g_AlwaysOnTop ? "AlwaysOnTop": " "
;}
...
IE_DownloadSource(url, pathToFile="") {
   ...
   While   pwb.readyState!=4 || pwb.document.readyState!="complete" || pwb.busy
      Sleep 50
	if   !(pwb :=   ComObj(9,ComObjQuery(pdsp,IID,IID),1))
      return   False
   source 	:=   pwb.document.documentElement.outerHTML, pwb.Quit()
   pwb 		:=   ""

return   source
}

IE_GetSourceFromWindow(hWnd="") {

	static	h, idObject :=	-4, IID :=   "{332C4427-26CB-11D0-B483-00C04FD90119}"
	if	!h
	...
	if	DllCall("oleacc\AccessibleObjectFromWindow", "Ptr", hWnd, "UInt", idObject&=0xFFFFFFFF
			, "Ptr", -VarSetCapacity(CLSID,16)+NumPut(idObject==0xFFFFFFF0?0x46000000000000C0:0x719B3800AA000C81
			,NumPut(idObject==0xFFFFFFF0?0x0000000000020400:0x11CF3C3D618736E0,CLSID,"Int64"),"Int64"), "Ptr*", pacc)=0

return	ComObj(9,ComObjQuery(ComObjEnwrap(9,pacc),IID,IID),1).document.documentElement.outerHTML
}
...
; Buttons                    	;{
	...
	Gui Add, Button, % "hWndhBtnOpts gShowSettingsDlg vBtn1 	x10 y" (432+plusH) " w24 h24"
	Gui Add, Button, % "gShowFindDlg vBtn2 								x39 y" (432+plusH) " w84 h24", &Find...
	Gui Add, Button, % "gShowTree vBtn3            						x128 y" (432+plusH) " w84 h24", &Tree...
	Gui Add, Button, % "gCopyToClipboard vBtn4 						x217 y" (432+plusH) " w84 h24", &Copy
	Gui Add, Button, % "gScreenshot vBtn5             						x306 y" (432+plusH) " w84 h24", &Screenshot
	Gui Add, Button, % "gWSReload vBtn6 									x395 y" (432+plusH) " w84 h24", &Reload
	SetButtonIcon(hBtnOpts, ResDir . "\Settings.ico")

	cp := GuiControlGetPos("Btn6", "")
	BtnH := cp.H
	cp := GuiControlGetPos("MsgDescription", "")
	MsgY := cp.Y
	hTab :=
	cp := GuiControlGetPos("Tab", "")
	TabY := cp.Y
	 hSpy :=WinExist("WinSpyM ahk_class AutoHotkeyGUI")
...
MessageInfo: ;{
	...
	GuiControl,, MsgDescription, % CbxMessages "(" Message "): " StrReplace(Descriptions[CbxMessages], "``n", "`n")
...
ToggleAlwaysOnTop: ;{
	...
	Gui, Spy: Submit, NoHide
	WinSet AlwaysOnTop, % g_AlwaysOnTop ? "On" : "Off", ahk_id %hSpyWnd%
    g_GuiAlwaysOnTop   := g_AlwaysOnTop ? "AlwaysOnTop": " "    ; this makes child gui`s AlwaysOnTop too
...
PostMsg: ;{
    ...
    GuiControl,, Result, % "msgNr: (" Message ")      Dec: " Result " | Hex: " GetHex(Result)

Return ;}

GetMessages(Class) { ; collects briefs descriptions from constants file
...
ShowXYWHDlg: ;{
    ...
    Gui XYWH: New, % "LabelXYWH " g_GuiAlwaysOnTop  ; *new* AlwaysOnTop if choosed
    Gui Font, s9, Segoe UI
    Gui Color, White
...
ShowSettingsDlg: ;{
    ...
    Gui Settings: New, % "LabelSettings hWndhSettingsDlg -MinimizeBox OwnerSpy " g_GuiAlwaysOnTop ; *new*
...
ShowFindDlg: ;{
    ...
    If (FindDlgExist) {
        ...
        Gui Find: New, % "LabelFind hWndhFindDlg " g_GuiAlwaysOnTop
        Gui Font, s9, Segoe UI
        Gui Color, White
...
ShowTree: ;{
    ...
    If (WinExist("ahk_id" . hTreeWnd)) {
        ...
        Gui Tree: New, % "LabelTree hWndhTreeWnd +Resize " g_GuiAlwaysOnTop
        SetWindowIcon(hTreeWnd, "shell32.dll", 42)
...
ShowMenuViewer: ;{                                                                                 	; Based on a script written by Lexicos
    ...
    If (MenuViewerExist) {
        ...
        Gui MenuViewer: New, % "+LabelMenuViewer +hWndhMenuViewer +Resize " g_GuiAlwaysOnTop
        Gui Font, s9, Segoe UI
        Gui Color, 0xF1F5FB
...
Control_GetFont(hWnd, ByRef Name, ByRef Size, ByRef Style, IsGDIFontSize := 0) {                 	; www.autohotkey.com/forum/viewtopic.php?p=465438#465438
...
ControlGetTabs(hTab) {                                                         	; https://autohotkey.com/board/topic/70727-ahk-l-controlgettabs/
...
GetDec(hwnd) {                                                                                                                    	;-- Hexadecimal to decimal
...
GetHex(hwnd) {                                                                                                                    	;-- decimal to hexadecimal
...
GetMonitorIndexFromWindow(windowHandle, ByRef MonStats:="") {
	...
	if (monitorHandle := DllCall("MonitorFromWindow", "uint", windowHandle, "uint", 0x2))
		...
		Monstats.monitorLeft   		:= NumGet(monitorInfo,  4, "Int")
		MonStats.monitorTop      	:= NumGet(monitorInfo,  8, "Int")
		MonStats.monitorRight    	:= NumGet(monitorInfo, 12, "Int")
		MonStats.monitorBottom 	:= NumGet(monitorInfo, 16, "Int")
		MonStats.workLeft      		:= NumGet(monitorInfo, 20, "Int")
		MonStats.workTop         	:= NumGet(monitorInfo, 24, "Int")
		MonStats.workRight     		:= NumGet(monitorInfo, 28, "Int")
		MonStats.workBottom    	:= NumGet(monitorInfo, 32, "Int")
		MonStats.isPrimary     		:= NumGet(monitorInfo, 36, "Int") & 1
		...
		MonStats.MonIndex := monitorIndex
...
GuiControlGetPos(ControlID:="", GuiName:="") {                	;-- ControlGet as function returns object
	GuiControlGet, ctl, % (GuiName ? GuiName ": " : "") "Pos", % ControlID
Return {"X":ctlX,"Y":ctlY,"W":ctlW,"H":ctlH}
}

TaskbarHeight(MonNr:=1) {
	hMSTaskList := WinExist("ahk_class Shell_TrayWnd")
	TaskList := GetWindowInfo(hMSTaskList)
return TaskList.H
}
